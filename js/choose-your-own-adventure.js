...
var runStory = function runStory(){
    var choice = prompt( story.start.text );

    if( choice === "north" ){
        choice =  prompt( story.north.text );

        if( choice === "treehouse" ){
            document.querySelector( "#output" ).textContent = story.treehouse.text;
        }
        ...
        choice =  prompt( story.south.text );
